defmodule Nectar.Order do
  ...
  def confirmation_changeset(model, params \\ %{}) do
    model
    |> cast(params, @required_fields ++ @optional_fields)
    |> validate_current_state("created")
    |> set_state("confirmed")
  ...
  defp validate_current_state(model, state) do
    current_state = get_field(model, :state)
    if(current_state == state) do
      model
    else
      add_error(model, :state, "Cannot perform the opration with the current state: #{current_state}")
    end
  end
